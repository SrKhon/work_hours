...
def tbl_month(request, slug):
    months = Month.objects.all()  # To list the months
    month = WorkDays.objects.filter(month__slug=slug)  # To display class fields
    current_month = Month.objects.get(slug__iexact=slug)  # To display current Month
